...
class Mysql {
    ...
    //获取单条记录
    public function row_array($sql, $fetch_mode = MYSQLI_ASSOC) {
        $this->query($sql, 1);
    ...
    public function cache_row($sql, $reload = false) {
        $this->reload   = $reload;
        $sql    = $this->get_query_sql($sql, 1);
        return $this->get_cache($sql, 'row_array');
    }
 
    //返回所有的结果集
    public function result_array($sql, $limit = null, $fetch_mode = MYSQLI_ASSOC) {
